...
\begin{figure}
\centering
\includegraphics[width=0.9\textwidth]{files/2015-08-14_14-43-31_faui1-246_timeout}
\caption{Frequency of durations needed for receiving a peer message}
\label{timeout-calibration}
\end{figure}

To assess a mimimum timeout, a analysis with special configuration parameters was performed. Here the maximum time used for receiving a message was recorded for every peer contact. These durations were rounded and the number of occurrences for each analyzed in, resulting in the graph of figure \ref{timeout-calibration}. For this task \config{network\_timeout} was set to 30 seconds to achieve most unbiased results. The diagram shows, with a five second timeout the vast majority of messages will be received.
