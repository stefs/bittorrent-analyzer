...
\date{Erlangen, September 3, 2015}
\publishers{
  \begin{tabular}{rl}
...
When collecting all messages from a peer, a maximum of \numprint{256} messages are considered. This limit is in place, to prohibit infinite peer sessions. Only about \numprint[\%]{0.1} of evaluations reached this maximum on the main analysis pass, others stopped because of the timeout or another error.
...
When the download progress of a peer changes between two visits, a speed value can be calculated. This has been done for all peers during the analysis, while only keeping the maximum speed measured between two visits per peer. Figure \ref{speed-boxes} shows the results sorted by the number of unique peers. The median speed is mostly about \numprint[kB/s]{100}, while the majority of measurements are lying between \numprint[kB/s]{10} and \numprint[MB/s]{1}. Standing out are China and Trinidad and Tobago with a median speed of nearly \numprint[MB/s]{10}. Most peers come from the United States, followed by Russia, the Philippines, the United Kingdom and South Korea. Speed values are drawn in a map in figure \ref{speed-map}. The location of peers is presented in the map of figure \ref{loc-map}.
...
\label{total-history}
\ohead{\rightmark}
\newcounter{TorrentPage}

\stepcounter{TorrentPage}
