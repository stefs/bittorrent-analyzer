...
\end{messcall}
\end{sequencediagram}
\caption[Sequence diagram of the BitTorrent Download Analyzer, part 1]{First part of the sequence diagram of the BitTorrent Download Analyzer. All loops are actually running in parallel. One additional threads which is not shown here writes monitoring statistics such as system load, memory consumption and queue lengths to the database.}
\label{analyzer1}
\end{figure}
...
\paragraph{util.py} The util file provides utility methods which are used in all other modules. They are not specific for the application in this software.

\paragraph{config.py} The config file provides detailed parameters for the performed analysis.
...
\section{Requesting Peers}
For every registered torrent, an own thread performs announce and scrape requests in an interval defined in \config{tracker\_request\_interval}. While announce requests to collect peers are sent to every tracker of a torrent, scrape requests are only performed on the main tracker from the \texttt{announce} key of the metainfo file. From the scrape request, the three given values of seeders, leechers and completed downloads are recorded. To monitor the operation of the torrent threads, any errors are written in an extra file with the suffix \texttt{\_peer-error.txt} in the output directory.

Equally are peers collected from the DHT network, although only in a single thread. As specified in \config{dht\_request\_interval}, requests for peers are sent for each info hash. Peers received from both sources are placed in a peer queue, where they are actively evaluated later. Only new unique peers are placed in the peer queue. The number of unique and doubly received peers as well as data from scrape requests is stored in the database's \emph{request} table.
...
\subsection{Evaluate Download Progress}
