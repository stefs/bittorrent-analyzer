...
While there are no legal restrictions on using BitTorrent in general, the download of content without permission of the author or rightholder is considered an illegitimate reproduction according to the German Copyright Act \cite[art.~15\,(1),~16]{urhg}. The common exception of private copying \cite[art.~53]{urhg} is not applicable here, since the source is obviously illegitimate.

Even more serious is the upload process always involved in BitTorrent. Illegitimate distribution of proprietary content may be sentenced with imprisonment or a fine \cite[art.~106]{urhg}. More common and often abused \cite{abmahnung} is the system of special notifications \cite[art.~97a]{urhg}, sent from rightholders to assumed copyright infringers. These warning letters are supposed to settle the controversy extrajudicial in exchange of a fee. Entitlement of the rightholder to indemnity and expense allowance exists \cite[art.~97]{urhg}.

\subsubsection{Collecting Statistics}
Privacy is regulated by the Federal Data Protection Act \cite{bdsg} in Germany. It introduces a concept of personal data which includes data that can be related to an individual with additional and accessible sources \cite{}. The connection between IP addresses and individuals can be provided by the \emph{right of information} as admitted in the Copyright Act \cite[art.~101]{urhg}. If IP addresses are within the definition of personal data regarding this law is disputed \cite{}. Anonymizing IP addresses by dropping higher bits is advised \cite{} and used in this thesis' software tool.
