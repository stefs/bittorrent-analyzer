...
Peers were tracked up to a download progress of \numprint[\%]{98}. Since the number of pieces on the first and last contact for every peer were recorded, the number of confirmed downloads can be extracted by filtering for peers with a progress on the first visit below and on the last visit above the threshold. Different threshold values were tested in figure \ref{thresh-chg}. The maximum value is nearly identical between \numprint[\%]{75} and  \numprint[\%]{98}, with declining numbers for lower values. The value of \numprint[\%]{98} will be used for calculating confirmed download numbers, in order to only register complete downloads.
