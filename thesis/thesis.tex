...
\usepackage[vmargin=3.5cm,hmargin=3cm]{geometry}
...
\texttt{info/pieces} & For each piece a SHA-1 hash value calculated. Their raw bytes are concatenated and stored here. The total number of pieces can be derived from this value by dividing it's length by 20, since a SHA-1 hash is 20 bytes. \\
...
\bottomrule
\end{tabular}
\caption{Compilation of torrents from the most popular torrent sites}
\label{torrents}
\end{table}
