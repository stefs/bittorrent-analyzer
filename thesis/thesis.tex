...
To assess a mimimum timeout, the maximum used time between messages was recorded for every peer session. An average of these values was calculated and stored every five minutes, according to \config{statistic\_interval}. Only for this timeout estimation run \config{network\_timeout} was set to 30 seconds to achieve most unbiased results. As can be seen in \ref{timeout-test}, five seconds is about \_ times above the total average of \_ seconds.
