...
\begin{quote}
% Motivation
BitTorrent ist die meistgenutzte Filesharing-Technologie, wobei die Schhädlichkeit für die Kreativindustrie diskutiert wird. Jedoch kann man den Umfang an heruntergeladenen Kopien nur abschätzen.
% Problem statement
...
% Approach
Diese Arbeit wird diese Methode erweitern, indem jeder Peer wiederhohlt kontaktiert und der Download-Fortschritt mit Hilfe des BitTorrent-Protokolls extraihert wird. Es wurde ein Programm geschrieben, das dies tut, nachdem es Addressen von Trackern und dem DHT-Netzwerk sammelt, und eingehende Verbindungen annimmt. Ein bestätigter Download wurde registriert, wenn ein Peer den Grenzwert von \numprint[\%]{98} überschreitet.
% Results
Über 34 Stunden und 19 Torrents konnten für kleine beziehungsweise große Torrents zwischen \numprint[\%]{9} und \numprint[\%]{55} der von Haupttrackern in Scrape-Antworten gemeldeten Downloads bestätigt werden. Für weniger als \numprint[\%]{2} aller eindeutigen Addressen wurde ein Download bestätigt.
% Conclusions
Wiederhohlte Auswertungen von Peers sind oft fehlgeschlagen, was diese Zahlnen wesentlich reduziert. Die verwendete Implementierung muss weiter angepast werden, um bessere Ergebnisse zu erhalten.
\end{quote}
\vspace*{\fill}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\caption[Development of confirmed and reported downloads per torrent set]{Development of confirmed and server reported downloads per hour and torrent set. The timestamp for a confirmed download is the time of first contact with the peer. Analysis duration was \range.}
