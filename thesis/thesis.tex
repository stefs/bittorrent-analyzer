...
Key & Value \\
\midrule
\texttt{announce} & URL of the tracker server, usually in the form of \nolinkurl{http://<host>:<port>/announce} \\
\texttt{info/name} & Optional file or directory name \\
\texttt{info/piece length} & Number of bytes per piece \\
\texttt{info/pieces} & Concatenation of raw SHA-1 hash values for every individual piece \\
\texttt{info/length} & Only present if the torrent is a single file; file size in bytes; not used in this project at all \\
\texttt{info/files/length} & Only present if the torrent contains multiple files; list of dictionaries, one per file; not used in
this project at all --- File size in bytes \\
\texttt{info/files/path} & List of subdirectory names and filename representing a path \\
\bottomrule
\end{tabularx}
\caption{Structure of dictionaries in the bencoded metainfo file format}
\label{metainfo-file}
\end{table}

Metadata about a downloadable file set is stored using bencoding as described above. These so called \emph{metainfo files} are using the \texttt{.torrent} file name extension. Values are grouped in dictionaries for easy reference as listed in table \ref{metainfo-file}.

These files allow users to identify torrents, since they mostly contain a human readable description as well as cryptographic hash values on the torrent's content. Additionally the URL of a tracker server is stored, allowing BitTorrent clients to gain information about other peer's addresses and participate in the network.
