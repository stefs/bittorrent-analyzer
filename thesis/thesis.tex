...
\begin{itemize}
  \item Set A: The 6 most popular torrents below \numprint[GB]{1}
  \item Set B: The 6 most popular torrents between \numprint[GB]{1} and  \numprint[GB]{10}
  \item Set C: The 6 most popular torrents above \numprint[GB]{10}
\end{itemize}
...
\label{unique-peers}
\end{table}
% SQL Total: select sum(received\_peers) from request where source='tracker';
% SQL Unique: select sum(received\_peers) - sum(duplicate\_peers) from request where source='<source>';
...
The progression of received peer addresses during the analysis period is shown in figure \ref{request-history}. Again, this is a summary for all torrents. The ratio between new and duplicate received peer address information is highest in the first few hours* and then settles at a constant value. The overall high rates of duplicate peers indicate that the majority of the peers collectable from theses sources were received. To estimate the success of tracker announce requests, encountered errors were counted. Figure \ref{tracker-resp} shows no problems.
...
[scrape only gives hint, only from main tracker, not the truth]

\begin{figure}
\centering
\includegraphics[width=\textwidth]{../result/2015-08-27_21-03-31_faui1-246_download_set_A_(7,8,15)}
\includegraphics[width=\textwidth]{../result/2015-08-27_21-03-31_faui1-246_download_set_B_(1,2,3,6,14)}
\includegraphics[width=\textwidth]{../result/2015-08-27_21-03-31_faui1-246_download_set_C_(4,10,12,13)}
