...
\subsection{Usage}
In order to study the download behaviour of BitTorrent peers, a tool named \emph{BitTorrent Download Analyzer} was written in Python 3. It is structured in the main script, an application module, five helper modules and an utility module, which will be described in detail. The behaviour of the program can be altered using the dedicated configuration file named \texttt{config.py}, which contains several variables. For simplification these variables will be referred to with the prefix \texttt{config.} in the following, for example \texttt{config.parameter\_one} translates to variable \texttt{parameter\_one} in the configuration file. The main script, named \texttt{main.py} can be controlled by using the following command line options.
...
\begin{description}
  ...
  \item[\texttt{-{}-dht}] Integrate and control an already running \emph{pymdht} \cite{pymdht} DHT node using Telnet. The UDP port on which the node is running and the Telnet port on \texttt{localhost} where \emph{pymdht} can be controlled are given via \texttt{config.dht\_node\_port} and \texttt{config.dht\_control\_port} respectively.
...
Beforehand the torrents to be analyzed must be prepared. Since magnet links\footnote{} and torrent files\footnote{} are supported there are two major ways to do this. All torrent files must be placed in a common directory, specified by \texttt{config.input\_path}. They are detected by their \texttt{.torrent} file name extension. All magnet links to be considered must be placed in a file defined by \texttt{config.magnet\_file}, one per line.

\subsection{Dependencies}
There are a few external dependencies, which are all free and open-source software. The \emph{BencodePy} \cite{bencodepy} project by \textsc{Eric Weast} provides an encoder and decoder for bencoding messages and values. The \emph{Object Relational Mapper} of \emph{SQLAlchemy} \cite{sqlalchemy} is used to store evaluation results in the \emph{SQLite} \cite{sqlite} database format. The \emph{GeoIP2 API} \cite{geoip2-api} is used to perform IP geolocation lookups in the \emph{GeoLite2 City Database} \cite{geolite2-city-database}. This database is provided by \textsc{MaxMind, Inc.} under the \emph{Creative Commons Attribution-ShareAlike 3.0 Unported License}.

In order to run a dedicated DHT node, the tool \emph{pymdht} \cite{pymdht} by \textsc{Raul Jimenez} is used. It must me started seperately and is controlled automatically using a localhost Telnet connection. It could not be integrated directly, since it is written in Python version 2. The desired UDP node port and the Telnet control port must be given as arguments and should reflect the values written in the \emph{BitTorrent Download Analyzer's} configuration file. The typical command used here is \texttt{run\_pymdht\_node.py -{}-port=17000 -{}-telnet-port=17001}. It is sensible to check whether \emph{pymdht} has crashed before and after each analysis run to ensure complete results. 
...
Beforehand the torrents to be analyzed must be prepared. 
...
\paragraph{\texttt{config.network\_timeout}}
...
To assess a reasonable timeout, the maximum used time between messages was recorded for every peer session. An average of these values was calculated and stored every five minutes, according to \texttt{config.statistic\_interval}. Only for this timeout estimation run \texttt{config.network\_timeout} was set to 30 seconds to achive most unbiased results. As can be seen in \cite{timeout-test}, five seconds are \_ times above the total average of \_ seconds.
