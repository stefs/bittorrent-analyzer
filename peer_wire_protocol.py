...
class PeerSession:
	...
	def __init__(self, peer_ip, timeout):
		# Store peer ip data and timeout parameter
	...
	def send_bytes(self, data):
		...
		while total_sent_count < len(data):
			try:
				...
				raise PeerError('Could not send data: ' + str(err))
			if sent_count == 0:
		...
		if bytes_to_receive > 0:
			...
			while received_bytes < bytes_to_receive:
				try:
					...
					raise PeerError('Could not read from socket: ' + str(err))
				if buffer == b'':
			                raise PeerError('Socket connection broken')
	...
	def receive_handshake(self, info_hash):
		...
		# Parse peer id
		peer_id = handshake_tuple[3]
		logging.info('ID of connected peer is ' + str(peer_id))
